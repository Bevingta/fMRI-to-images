...
with open('data/regression_weights_reducedbs/subj{:02d}/vdvae_regression_weights.pkl'.format(sub),"rb") as f:
...
nsd_features = np.load('data/extracted_features_reducedbs/subj{:02d}/nsd_vdvae_features_31l.npz'.format(sub))
...
print('vae', pred_vae.shape, pred_vae)
np.save('data/predicted_features_reducedbs/subj{:02d}/nsd_vdvae_nsdgeneral_roi_sub{}_31l_alpha50k.npy'.format(sub,sub),pred_vae)

# Generate CLIP-Text Features

with open('data/regression_weights_reducedbs/subj{:02d}/cliptext_regression_weights.pkl'.format(sub),"rb") as f:
    datadict = pickle.load(f)
    reg_w = datadict['weight']
    reg_b = datadict['bias']
...
print('clipt', pred_clipt.shape, pred_clipt)
np.save('data/predicted_features_reducedbs/subj{:02d}/nsd_cliptext_roi_nsdgeneral.npy'.format(sub),pred_clipt)

# Generate CLIP-Vision Features

with open('data/regression_weights_reducedbs/subj{:02d}/clipvision_regression_weights.pkl'.format(sub),"rb") as f:
    datadict = pickle.load(f)
    reg_w = datadict['weight']
    reg_b = datadict['bias']
...
print('clipv', pred_clipv.shape, pred_clipv)
np.save('data/predicted_features_reducedbs/subj{:02d}/nsd_clipvision_roi_nsdgeneral.npy'.format(sub),pred_clipv)
