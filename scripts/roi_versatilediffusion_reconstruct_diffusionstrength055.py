...
parser.add_argument("-mix_str", "--mix_str",help="Mixing Strength",default=1)
...
pred_text = np.load('data/predicted_features_reducedbs/subj{:02d}/nsd_cliptext_roi_nsdgeneral.npy'.format(sub))
pred_text = torch.tensor(pred_text).half().cuda(0)

pred_vision = np.load('data/predicted_features_reducedbs/subj{:02d}/nsd_clipvision_roi_nsdgeneral.npy'.format(sub))
...
res_dir = 'results/versatile_diffusion_reducedbs/subj{:02d}/roi/'.format(sub)
...
for im_id in range(len(pred_vision)):
    ...
    zim = Image.open('results/vdvae_reducedbs/subj{:02d}/roi/{}.png'.format(sub,im_id))
    ...
    x[0].save('results/versatile_diffusion_diffusionstrength055/subj{:02d}/roi/{}.png'.format(sub,im_id))
