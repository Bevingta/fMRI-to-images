...
parser.add_argument("-bs", "--bs",help="Batch Size",default=3)
...
pred_latents = np.load('data/predicted_features_reducedbs/subj{:02d}/nsd_vdvae_nsdgeneral_roi_sub{}_31l_alpha50k.npy'.format(sub,sub))
...
print('input_latent', input_latent)
...
res_dir = 'results/vdvae_reducedbs/subj{:02d}/roi/'.format(sub)
...
for i in range(1):
  ...
  print('samp', len(samp), len(samp[0]), samp[0][0])
  print('1 done') # ADDED BY ANDREA

  px_z = ema_vae.decoder.forward_manual_latents(len(samp[0]), samp, t=None)
  print('px_z', px_z)
  print(ema_vae.decoder)
  print('2 done') # ADDED BY ANDREA

  sample_from_latent = ema_vae.decoder.out_net.sample(px_z)
  print(len(sample_from_latent))
  print('3 done') # ADDED BY ANDREA

  upsampled_images = []
  for j in range(len(sample_from_latent)):
      im = sample_from_latent[j]
      print(im)
      print('4.1 done') # ADDED BY ANDREA
      im = Image.fromarray(im)
      print('4.2 done') # ADDED BY ANDREA
      im = im.resize((512,512),resample=3)
      im.save('results/vdvae_reducedbs/subj{:02d}/roi/{}.png'.format(sub,i*batch_size+j))
