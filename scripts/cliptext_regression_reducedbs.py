...
train_clip = np.load('data/extracted_features_reducedbs/subj{:02d}/nsd_cliptext_train.npy'.format(sub))
test_clip = np.load('data/extracted_features_reducedbs/subj{:02d}/nsd_cliptext_test.npy'.format(sub))

## Regression
num_samples,num_embed,num_dim = train_clip.shape
print(num_samples,num_embed,num_dim, num_voxels, num_train, num_test) #ADDED BY ANDREA
...
np.save('data/predicted_features_reducedbs/subj{:02d}/nsd_cliptext_predtest_nsdgeneral.npy'.format(sub),pred_clip)
...
with open('data/regression_weights_reducedbs/subj{:02d}/cliptext_regression_weights.pkl'.format(sub),"wb") as f:
