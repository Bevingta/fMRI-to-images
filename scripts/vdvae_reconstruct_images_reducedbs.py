...
pred_latents = np.load('data/predicted_features_reducedbs/subj{:02d}/nsd_vdvae_nsdgeneral_pred_sub{}_31l_alpha50k.npy'.format(sub,sub))
ref_latent = stats
...
for i in range(int(np.ceil(len(test_images)/batch_size))):
  ...
  for j in range(len(sample_from_latent)):
      ...
      im.save('results/vdvae_reducedbs/subj{:02d}/{}.png'.format(sub,i*batch_size+j))
