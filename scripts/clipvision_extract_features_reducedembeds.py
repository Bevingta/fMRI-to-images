...
num_embed, num_features, num_test, num_train = 77, 768, len(test_images), len(train_images) #MODIFIED BY ANDREA, num_embed was 257.
...
with torch.no_grad():
    for i,cin in enumerate(testloader):
        ...
        test_clip[i] = c[0][:num_embed].cpu().numpy()
    
    np.save('data/extracted_features_reducedbs/subj{:02d}/nsd_clipvision_test.npy'.format(sub),test_clip)
        
    for i,cin in enumerate(trainloader):
        print(i)
        #ctemp = cin*2 - 1
        c = net.clip_encode_vision(cin)
        train_clip[i] = c[0][:num_embed].cpu().numpy()
    np.save('data/extracted_features_reducedbs/subj{:02d}/nsd_clipvision_train.npy'.format(sub),train_clip)
