...
import torch
print(torch.cuda.device_count()) #Added by andrea
...
parser.add_argument("-diff_str", "--diff_str",help="Diffusion Strength",default=0.55)
...
pred_text = np.load('data/predicted_features_reducedbs/subj{:02d}/nsd_cliptext_predtest_nsdgeneral.npy'.format(sub))
pred_text = torch.tensor(pred_text).half().cuda(0) #changed by andrea. was cuda(1)

pred_vision = np.load('data/predicted_features_reducedbs/subj{:02d}/nsd_clipvision_predtest_nsdgeneral.npy'.format(sub))
pred_vision = torch.tensor(pred_vision).half().cuda(0) #changed by andrea. was cuda(1)
...
for im_id in range(len(pred_vision)):
    ...
    zim = Image.open('results/vdvae_reducedbs/subj{:02d}/{}.png'.format(sub,im_id))
    ...
    utx = utx.cuda(0).half() #changed by andrea. was cuda(1)
    
    dummy = torch.zeros((1,3,224,224)).cuda(0)
    uim = net.clip_encode_vision(dummy)
    uim = uim.cuda(0).half() #changed by andrea. was cuda(1)
    
    z_enc = z_enc.cuda(0) #changed by andrea. was cuda(1)
    ...
    sampler.model.model.diffusion_model.device='cuda:0' #changed by andrea. was cuda:1
    sampler.model.model.diffusion_model.half().cuda(0) #changed by andrea. was cuda(1)
    mixing = 1
    ...
    x[0].save('results/versatile_diffusion_diffusionstrength055/subj{:02d}/{}.png'.format(sub,im_id))
