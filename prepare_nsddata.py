...
print('KEYS',stim_order.keys()) # ADDED BY ANDREA.
print('LENGTH', len(stim_order)) # ADDED BY ANDREA.
print('SUBJECTIM',stim_order['subjectim'][:10]) #ADDED BY ANDREA.
print('MASTERORDERING',stim_order['masterordering'][:10]) #ADDED BY ANDREA.
print('LEN SUBJECTIM', len(stim_order['subjectim'])) #ADDED BY ANDREA.
print('LEN MASTERORDERING', len(stim_order['masterordering'])) #ADDED BY ANDREA.
...
for i, idx in train_im_idx: #ADDED BY DREW
    if idx > 7300:
        train_im_idx.pop(i)
...
stim = f_stim['imgBrick'][::10,::10,::10] # EDITED BY ANDREA. [::10,::10,::10] INSTEAD OF [:] 
...
num_train, num_test, im_dim,im_dim,im_c= 6500, 800, stim.shape[1], stim.shape[2], stim.shape[3] # ADDED BY ANDREA 
