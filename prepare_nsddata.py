...
for i,idx in enumerate(train_im_idx): 
    ...
    fmri_array[i] = fmri[i].mean(0) #EDITED BY ANDREA. fmri[i].mean(0) INSTEAD OF fmri[sorted(sig_test[idx])].mean(0)
    print(i)
...
for i,idx in enumerate(test_im_idx): 
    stim_array[i] = stim[idx]
    fmri_array[i] = fmri[i].mean(0) #EDITED BY ANDREA. frmi[i].mean(0) INSTEAD OF fmri[sorted(sig_test[idx])].mean(0).
    print(i)
