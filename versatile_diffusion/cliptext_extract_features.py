...
from model_zoo.vd import VD
from cfg_holder import cfg_unique_holder as cfguh
from cfg_helper import get_command_line_args, cfg_initiates, load_cfg_yaml
...
with torch.no_grad():
    ...
    for i, annots in enumerate(test_caps):
        ...
        test_clip_list.append(c.to('cpu').numpy().mean(0))

    # Concatenate the encoded features from all batches
    test_clip = np.stack(test_clip_list)

    # Save the concatenated features to disk
